...
echo [6] Configurando RunOnce para arrancar la Fase 1 de PowerShell... >> C:\Log_PPKG.txt
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce" /v "DespliegueFase1" /t REG_SZ /d "powershell.exe -ExecutionPolicy Bypass -WindowStyle Maximized -File \"C:\Deploy_Plenergy\Scripts\Fase1.ps1\"" /f >> C:\Log_PPKG.txt 2>&1

echo [7] Fin de la preparacion autonoma. El equipo se reiniciara pronto. >> C:\Log_PPKG.txt
exit